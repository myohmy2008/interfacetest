...
class HtmlReport:
    ...
    def generate_html(self,head, file):
            ...
            # 查询所有测试结果并记录到html文档
            query = ('SELECT case_id, http_method, request_name,interface_name, request_url,'
                          'request_param, test_desc, result, reason FROM test_result')
            self.cursor.execute(query)
            ...
            for row in query_result:
                tab1<< tr(td(int(row[0]), align='center') + td(row[1]) +
                              ...
                              td(row[4])+ td(row[5]) + td(row[6]) +                   #+ td(row[5])
